    ...
    for (RaftServerImpl followerServer : cluster.getFollowers()) {
      ...
      for (boolean heartbeat : new boolean[] { true, false }) {
        if (!(followerMetrics.getFollowerAppendEntryTimer(heartbeat).getMeanRate() > 0.0d)) {
          System.err.println("wangjie metric followerId:" + followerId +
              " followerMetrics:" + followerMetrics.hashCode() +
              " heartbeat:" + heartbeat +
              " timer:" + followerMetrics.getFollowerAppendEntryTimer(heartbeat).hashCode() +
              " meat rate:" + followerMetrics.getFollowerAppendEntryTimer(heartbeat).getMeanRate() +
